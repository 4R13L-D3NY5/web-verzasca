      ...
      <div class="relative mt-3 w-full overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <tr class="color-bg border border-slate-200">
          ...
          <div class="mb-2">
            ...
            <span>{{ $base->capacidad }} ml</span>
          </div>
          <div class="mb-2">
            <span class="font-semibold block">Preforma:</span>
            <span>{{ $base->preforma->insumo ?? 'N/A' }}</span>
          </div>
          <div>
            <span class="font-semibold block">Sucursal:</span>
            @forelse ($base->existencias as $existencia)
        <span>{{ $existencia->sucursal->nombre ?? 'Sucursal Desconocida' }}:
        {{ number_format($existencia->cantidad) }}</span><br>
      @empty
    <span class="text-xs text-gray-500">Sin stock registrado</span>
  @endforelse
            <strong class="p-text block mt-1">Total Stock:
            {{ number_format($base->existencias->sum('cantidad')) }}</strong>
          </div>
          </td>
      ...
      <div class="center-col">
        ...
        <dl class="grid grid-cols-2 gap-4">
          ...
          <dd class="mt-1 text-sm p-text">
            {{ $baseSeleccionada->preforma ? $baseSeleccionada->preforma->insumo . ' [' . $baseSeleccionada->preforma->capacidad . 'ml]' : 'Ninguna' }}
          </dd>
          </div>
