<div class="dark:bg-gray-900 text-white p-4 flex justify-center">
  <div class="w-full max-w-screen-xl grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 gap-6">
      <div class="relative flex w-full flex-col rounded-xl bg-white text-gray-700 shadow-md">
          <!-- Título bonito -->
          <h6 class="text-center text-xl font-bold text-gray-800 dark:text-white mb-4 px-4">Gestión de Proveedores</h6>
          ...
          <div class="flex overflow-hidden bg-white border divide-x rounded-lg rtl:flex-row-reverse dark:bg-gray-900 dark:border-gray-700 dark:divide-gray-700">
              <button title="Registrar proveedor" wire:click='abrirModal("create")' class="group cursor-pointer outline-none hover:rotate-90 duration-300">
                          ...
                          <tr class="border-b border-gray-200 dark:border-gray-700">
                              <td class="px-6 py-4 font-medium text-gray-900 bg-gray-50 dark:text-white dark:bg-gray-800">
                                  <div class="flex flex-col">
                                      ...
                                      <span class="text-gray-500 dark:text-gray-400 text-xs">{{ $proveedor->nombreContacto ?? 'N/A' }}</span>
                                  </div>
                              </td>
                              <td class="bg-gray-50 dark:bg-gray-800 w-40 text-right p-2">
                                  <div class="flex justify-end">
                                      <button wire:click="editarProveedor({{ $proveedor->id }})" class="text-gray-600 hover:text-gray-800 mx-1 transition-all duration-200 ease-in-out hover:rotate-12 focus:outline-none focus:ring-2 focus:ring-gray-500 rounded-full">
                                      ...
                                      <button wire:click="verDetalle({{ $proveedor->id }})" class="text-blue-500 hover:text-blue-600 mx-1 transition-transform duration-200 ease-in-out hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-full">
                                          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <div class="fixed inset-0 z-10 w-screen overflow-y-auto flex items-center justify-center">
              <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all w-full max-w-md mx-4 sm:mx-0">
                  <div class="px-4 py-4 sm:px-5 sm:py-4">
                      <div class="w-full">
                          <h3 class="text-base font-semibold text-gray-900 dark:text-white" id="modal-title">
                              {{ $accion === 'create' ? 'Registrar Proveedor' : 'Editar Proveedor' }}
                          </h3>
                          <div class="mt-2">
                              <div class="grid grid-cols-1 gap-3">
                                  <div>
                                      <label class="label1">Razón Social</label>
                                      <input type="text" wire:model="razonSocial" class="input1">
                                      @error('razonSocial') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                                  </div>
                                  <div>
                                      <label class="label1">Nombre de Contacto</label>
                                      <input type="text" wire:model="nombreContacto" class="input1">
                                      @error('nombreContacto') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                                  </div>
                                  <div>
                                      <label class="label1">Dirección</label>
                                      <input type="text" wire:model="direccion" class="input1">
                                  ...
                                  <div>
                                      ...
                                      <select wire:model="servicio" class="select1">
                                  ...
                                  <div>
                                      ...
                                      <textarea wire:model="descripcion" class="input1"></textarea>
                                      @error('descripcion') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                                  </div>
                                  <div>
                                      <label class="label1">Precio</label>
                                      <input type="number" step="0.01" wire:model="precio" class="input1">
                                      @error('precio') <span class="text-red-600 text-xs">{{ $message }}</span> @enderror
                                  </div>
                                  <div>
                                      <label class="label1">Tiempo de Entrega</label>
                                      <input type="text" wire:model="tiempoEntrega" class="input1">
                  ...
                  <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-5">
                      <button type="button" wire:click="guardarProveedor" class="inline-flex w-full justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 sm:ml-2 sm:w-auto">Guardar</button>
  ...
  @if ($detalleModal)
      <div class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
          ...
          <div class="fixed inset-0 z-10 w-screen overflow-y-auto flex items-center justify-center">
              <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left shadow-xl transition-all w-full max-w-md mx-4 sm:mx-0">
                  <div class="px-4 py-4 sm:px-5 sm:py-4">
                      <h3 class="text-base font-semibold text-gray-900 dark:text-white" id="modal-title">Detalles del Proveedor</h3>
                      <div class="mt-4">
                          <dl class="grid grid-cols-1 gap-4">
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Razón Social</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->razonSocial }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Nombre de Contacto</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->nombreContacto ?? 'N/A' }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Dirección</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->direccion }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Teléfono</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->telefono }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Correo</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->correo }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Tipo</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ ucfirst($proveedorSeleccionado->tipo) }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Servicio</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ ucfirst($proveedorSeleccionado->servicio) }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Descripción</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->descripcion }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Precio</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ number_format($proveedorSeleccionado->precio, 2) }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Tiempo de Entrega</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->tiempoEntrega }}</dd>
                              </div>
                              <div>
                                  <dt class="text-sm font-medium text-gray-500 dark:text-gray-300">Estado</dt>
                                  <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $proveedorSeleccionado->estado ? 'Activo' : 'Inactivo' }}</dd>
                              </div>
                          </dl>
                      </div>
                  </div>
