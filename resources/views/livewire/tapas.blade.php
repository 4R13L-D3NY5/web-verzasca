<div class="text-white p-2 mt-10 flex justify-center">
  <div class="w-full max-w-screen-xl grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 gap-6">
    <div class="relative flex w-full flex-col rounded-xl  text-gray-700 shadow-md">
      <h6 class="text-center text-xl font-bold text-gray-800 dark:text-white mb-4 px-4">Gestión de Tapas</h6>
      <div class="flex overflow-hidden border divide-x rounded-lg rtl:flex-row-reverse dark:bg-gray-900 dark:border-gray-700 dark:divide-gray-700">
        <button title="Registrar tapa" wire:click='abrirModal("create")' class="group cursor-pointer outline-none hover:rotate-90 duration-300">
              ...
              <tr class="border-b border-gray-200 dark:border-gray-700">
                <td class="bg-gray-50 dark:bg-gray-800 w-40 text-right p-2">
                  <div class="flex justify-end">
                    <button wire:click="abrirModal('edit', {{ $tapa->id }})" class="text-gray-600 hover:text-gray-800 mx-1 transition-all duration-200 ease-in-out hover:rotate-12 focus:outline-none focus:ring-2 focus:ring-gray-500 rounded-full">
                ...
                <td class="px-6 py-4">{{ $tapa->color }}</td>
                <td class="px-6 py-4">{{ $tapa->tipo }}</td>
                <td class="px-6 py-4">
                  <span class="text-{{ $tapa->estado ? 'green' : 'red' }}-500">{{ $tapa->estado ? 'Activo' : 'Inactivo' }}</span>
                </td>
              </tr>
            @empty
              <tr>
                <td colspan="4" class="text-center py-4 text-gray-600 dark:text-gray-400">No hay tapas registradas.</td>
              </tr>
            @endforelse
          </tbody>
        </table>
        ...
        <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-full max-w-md mx-4 sm:mx-0">
          <div class="px-6 py-6 flex flex-col items-center">
            ...
            <div class="mt-4 w-full flex flex-col gap-3 max-h-[80vh] overflow-y-auto">
              ...
              <div class="input-container">
                <input placeholder="Tipo" type="text" wire:model="tipo" class="input-field">
              </div>
